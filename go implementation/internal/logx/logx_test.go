...
func TestPeerRedactionByLevel(t *testing.T) {
	addr := &net.TCPAddr{IP: net.IPv4(140, 170, 23, 9), Port: 443}

	SetLevel(slog.LevelDebug)
	if got := Peer(addr).Value.String(); got != "140.170.23.9:443" {
	...
	for _, l := range []slog.Level{slog.LevelInfo, slog.LevelWarn, slog.LevelError} {
		SetLevel(l)
		if got := Peer(addr).Value.String(); got != "140.170.*.*:443" {
